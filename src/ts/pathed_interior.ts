...
export class PathedInterior extends Interior {
	...
	getTransformAtTime(dst: Matrix4, time: number) {
		...
		// Freeze the moving platform if the stopwatch is active
		if (this.level.stopWatchActive) {
			// Freeze platform at 1/2 of total path duration (or any fixed %)
			let totalTime = 0;
			for (let i = 0; i < this.markerData.length; i++) {
				totalTime += this.markerData[i].msToNext;
			}
			time = totalTime * 0.5; // Freeze at 50% point of motion
		}
