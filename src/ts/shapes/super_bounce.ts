...
export class SuperBounce extends PowerUp {
	...
	constructor(element: MissionElementItem) {
		...
		this.bounceRestitution = element.bouncerestitution ? MisParser.parseNumber(element.bouncerestitution) : 0.9; // Default bounce restitution 0.9 if not set
		this.superBounceTime = element.superbouncetime ? MisParser.parseNumber(element.superbouncetime) : 5000; // Default to 5000ms (5 seconds) if not set
	}
	...
	use() {
		if (this.level.marble.doubler) {
			...
			this.level.marble.enableSuperBounceDoubler(this.level.timeState, this.superBounceTime);
			this.level.marble.doubler = false; // Reset doubler
		}
		else {
			this.level.marble.enableSuperBounce(this.level.timeState, this.superBounceTime);
		}
