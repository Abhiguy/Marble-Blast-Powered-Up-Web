...
export abstract class MissionLibrary {
	...
	static async init() {
		...
		for (let mission of mbgMissions) {
			...
			else if (missionType === 'sandbox') this.goldSandbox.push(mission);
		...
		for (let i = 0; i < this.goldSandbox.length; i++) this.goldSandbox[i].initSearchString();
