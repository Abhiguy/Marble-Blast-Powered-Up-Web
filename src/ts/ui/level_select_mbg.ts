...
export class MbgLevelSelect extends LevelSelect {
	...
	initProperties() {
		...
		this.tabSandbox = document.querySelector('#tab-sandbox') as HTMLImageElement;
	...
	async init() {
		...
		setupTab(this.tabSandbox, MissionLibrary.goldSandbox);
		...
		this.setMissionArray(MissionLibrary.goldSandbox, false);
		...
		// Sandbox hover and down
		this.tabSandbox.addEventListener('mouseenter', () => {
			this.tabSandbox.src = './assets/ui/play/sandbox_h.png';
		});
		this.tabSandbox.addEventListener('mouseleave', () => {
			this.tabSandbox.src = './assets/ui/play/sandbox_n.png';
		});
		this.tabSandbox.addEventListener('mousedown', (e) => {
			if (e.button === 0) this.tabSandbox.src = './assets/ui/play/sandbox_d.png';
		});
		this.tabSandbox.addEventListener('mouseup', (e) => {
			if (e.button === 0) this.tabSandbox.src = './assets/ui/play/sandbox_h.png';
		});
	}
	...
	setMissionArray(arr: Mission[], doImageTimeout?: boolean) {
		...
		for (let elem of [this.tabBeginner, this.tabIntermediate, this.tabAdvanced, this.tabSandbox, this.tabCustom]) {
			elem.style.zIndex = "-1";
		}

		let index = [MissionLibrary.goldBeginner, MissionLibrary.goldIntermediate, MissionLibrary.goldAdvanced, MissionLibrary.goldSandbox, MissionLibrary.goldCustom].indexOf(this.currentMissionArray);

		let elem = [this.tabBeginner, this.tabIntermediate, this.tabAdvanced, this.tabSandbox, this.tabCustom][index];
		elem.style.zIndex = "0"; // Bring the tab to the front

		this.tabSandbox.style.zIndex = "0"; // Always 0
	}
