...
export abstract class HomeScreen {
	...
	constructor(menu: Menu) {
		...
		menu.setupButton(this.exitButton, this.exitSrc, () => {
			...
			if (!location.search.includes('app')) location.href = 'https://www.youtube.com/watch?v=Rs9CyQGphmQ&list=RDRs9CyQGphmQ&start_radio=1'; // Internet veterans will recognize this link
		});
	...
	async init() {
		...
		let latestVersion = /(^|\n)## (\d+\.\d+\.\d+)/.exec(text)[2];
		this.version.textContent = `MBPUW v${latestVersion}`;
