...
export class AudioManager {
	...
	updatePositionalAudio(time: TimeState, listenerPos: Vector3, listenerYaw: number) {
		...
		for (let source of this.audioSources) {
			...
			let maxDistance = source.maxDistance ?? 30;
			source.gain.gain.setValueAtTime(Util.clamp(1 - distance / maxDistance, 0, 1) * source.gainFactor, this.currentTime);
...
export class AudioSource {
	...
	loop = false;
	playbackRate = 1;
	maxDistance = 30; // The maximum distance at which the sound can be heard
