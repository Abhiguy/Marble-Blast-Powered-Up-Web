...
export abstract class Hud {
	...
	async load() {
		...
		imagePathsToLoad.push(...["ready.png", "set.png", "go.png", "outofbounds.png", "powerup.png", "x2.png", "face.png", "timetravel.png"].map(x => this.menu.uiAssetPath + "game/" + x));
		// imagePathsToLoad.push(...["ready.png", "set.png", "go.png", "outofbounds.png", "powerup.png", "x2.png", "face.png"].map(x => this.menu.uiAssetPath + "game/" + x));
	...
	renderHud(timeState: TimeState) {
		...
		const margin = 4; // Space between main clock and TT timer

		// Draw the Time Travel Timer...Don't show the time travel timer if stopwatch is active...
		if (state.level.currentTimeTravelBonus > 0 && !level.stopWatchActive) {
			let ttTime = Math.max(0, state.level.currentTimeTravelBonus);
			let ttString = Util.secondsToTimeString(ttTime / 1000);
			let yOffset = mainClockY + digitHeight + margin; // Directly below the main clock
		...
		if (level.marble.doubler)
			ctx.drawImage(x2Image, width - x2Image.width / 2 - powerUpBorder.width + 10, powerUpBorder.height - x2Image.height / 2 - 10);
	...
	drawNumbers(string: string, x: number, y: number, isClock: boolean, specialColor?: 'red' | 'green') {
		...
		for (let i = 0; i < string.length; i++) {
			...
			this.hudCtx.drawImage(image, x + currentX, y);
	...
	displayAlert(message: string, color: string) {
