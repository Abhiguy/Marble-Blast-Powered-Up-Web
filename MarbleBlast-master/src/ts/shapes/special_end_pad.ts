...
/** A Different type of Finish Pad for some specific levels */
export class SpecialEndPad extends Shape {
	...
	class Firework extends Scheduler {
		...
		constructor(level: Level, pos: Vector3, spawnTime: number) {
			...
			this.level.particles.createEmitter(fireworkSmoke1, this.pos); // Start the smoke
		...
		tick(time: number) {
			...
			for (let trail of this.trails.slice()) {
				...
				if (completion === 1) {
					...
					if (trail.type === 'red') {
						this.level.particles.createEmitter(redSpark1, pos);
					} else {
						this.level.particles.createEmitter(blueSpark1, pos);
					}
				}
			}
		}
		...
		spawnTrail(time: number) {
			...
			let emitter = this.level.particles.createEmitter((type === 'red')? redTrail1 : blueTrail1, this.pos);
			let randomPointInCircle = Util.randomPointInUnitCircle();
